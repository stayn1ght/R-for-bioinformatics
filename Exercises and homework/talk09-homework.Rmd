...
ggplot(swiss, aes(x = Catholic)) +
  ...
  labs(title = "Catholic", x = "Catholic") +
  theme_bw()
# 2. 用散点图显示 Eduction 与 Fertility 的关系并将表示两者关系的线性公式、相关系数和p值画在图的空白处。
# 2.1 基础做图函数
plot(swiss$Education, swiss$Fertility,
  xlab = "Education", ylab = "Fertility",
  main = "Education vs Fertility")
abline(lm(swiss$Fertility ~ swiss$Education), col = "red")
# 相关系数和p值画在图的空白处
text(40, 50,
  paste("r = ",
    round(cor(swiss$Education, swiss$Fertility), 2),
    "\n p < 10e-6 "))
cor.test(swiss$Education, swiss$Fertility)$p.value
# 2.2 ggplot2
ggplot(swiss, aes(x = Education, y = Fertility)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = "red") +
  labs(title = "Education vs Fertility", x = "Education", y = "Fertility") +
  theme_bw() +
...
plot(iris$Sepal.Length, iris$Petal.Length,
  col = iris$Species, xlab = "Sepal.Length",
  ylab = "Petal.Length",
...
```{r}
## 代码写这里，并运行；
(p1 <- ggplot(filter(starwars, !is.na(height) & !is.na(gender)),
    aes(x = gender, y = height)) +
  geom_boxplot() +
  geom_signif(comparisons = list(c("feminine", "masculine")),
...
```{r}
## 代码写这里，并运行；
# mtcars 增加 cyl ，加一组 468
my_mtcars <- bind_rows(mtcars, mutate(mtcars, cyl = 468))
(p3 <- ggplot(my_mtcars, aes(x = wt, y = mpg, color = factor(cyl))) +
  geom_point(size = 2.5) +
  geom_smooth(method = "loess") +
  facet_wrap(~ cyl, scales = "free", nrow = 2) +
...
```{r}
## 代码写这里，并运行；
mdf_mtcars <- mtcars %>% rownames_to_column() %>%
  arrange(cyl, mpg) %>%
  mutate(rowname = factor(rowname, levels = rowname)) %>%
  mutate(id = row_number(),
    angle = 90 - 360 * (id - 0.5) / n()) %>%
  select(rowname, mpg, cyl, angle)

(p4 <- ggplot(mdf_mtcars, aes(x = rowname)) +
  geom_bar(stat = "identity", aes(y = mpg, fill = factor(cyl))) +
  ...
  # Add the labels, using the mdf_mtcars dataframe
  geom_text(data = mdf_mtcars,
    aes(x = rowname, y = mpg + 28, label = rowname),
    color = "black", alpha = 0.9, size = 3,
    angle = mdf_mtcars$angle))
...
ggdraw() +
  ...
  draw_plot_label(c("A", "B", "C"), c(0, 0.5, 0.5),
    c(1, 1, 0.5), size = 15)
...
```{r}
## 代码写这里，并运行；
gridExtra::grid.arrange(p1, p2, p4,
  nrow = 2, layout_matrix = rbind(
    c(1, 1),
    c(2, 3)
))
