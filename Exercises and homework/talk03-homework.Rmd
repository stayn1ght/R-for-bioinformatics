...
read.table("data/Table3.txt", header = TRUE, sep = "\t", skip = 1, na.strings = c("","NA","--","*","**"))
# Table4.txt 身高数据 Height 中的值有逗号需要去掉
with(read.table("data/Table4.txt", header = TRUE, sep = "\t", 
  skip = 1, na.strings = c("","NA","--","*","**")), 
  Height <- as.numeric(gsub(",", "", Height)))

read.table("data/Table5.txt", header = TRUE, sep = ";", na.strings = c("","NA","--","*","**"))
...
# 读取 Table0.txt
read_tsv("data/Table0.txt", col_names = FALSE)
# 读取 Table1.txt，注意这里的 col_names = TRUE
read_tsv("data/Table1.txt", col_names = TRUE)
# 读取 Table2.txt，字符串引用符号为 /，跳过第一行
