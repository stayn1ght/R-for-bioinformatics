...
grades2 %>%
  ...
  mutate(course = case_when(
    min_grade == English ~ "English",
    min_grade == ComputerScience ~ "ComputerScience",
    min_grade == Biology ~ "Biology",
    min_grade == Bioinformatics ~ "Bioinformatics"
  )) %>%
  select(Name, course, min_grade)
# 2. 哪个职业的平均成绩最好？
grades2 %>%
  gather(key = "course", value = "grade", -Name, -Occupation) %>%
  group_by(Occupation) %>%
...
colnums <- select(Theoph, Subject:Dose) %>% ncol()
message("总共有", colnums, "列")
...
## 代码写这里，并运行；
Theoph %>%
    mutate(weight_cat = ifelse(Wt > 76.2, "Super-middleweight",
