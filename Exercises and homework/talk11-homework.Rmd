...
# 线性回归分析，检验 heart.disease 与 biking 和 smoking 的相关性
heart_lm <- lm(heart.disease ~ biking + smoking, data = heart_dat)
summary(heart_lm)
# 画出 heart.disease 与 biking 和 smoking 三者的关系
heart_bike <- ggplot(heart_dat, aes(x = biking, y = heart.disease)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +
  geom_text(aes(label = paste("y = ", round(heart_lm$coefficients[1], 2),
    " + ", round(heart_lm$coefficients[2], 2), " * x1 + ",
    round(heart_lm$coefficients[3], 2), " * x2", sep = "")),
    x = 5, y = 6.5, size = 5) +
  labs(title = "Heart Disease and Biking and Smoking", x = "Biking", y = "Heart Disease")

heart_smoke <- ggplot(heart_dat, aes(x = smoking, y = heart.disease)) +
