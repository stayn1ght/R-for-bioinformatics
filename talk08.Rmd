...
```{r eval=FALSE}
mtcars %>% split( .$cyl ) %>% map( ~ cor.test( .$wt, .$mpg ) ) %>% map_dbl( ~.$estimate );
# 最后一步 map_dbl( ~.$estimate ) 的意思是：
# 1. 从 map 返回的 list 中取出每个元素
# 2. 对每个元素取出 estimate 值
```
```
...
``` parallel ``` 包： 显示 CPU core 数量，将全部或部分 分配给任务。
...
3. ``` row = iter( d, by = "row" ) ``` ： 将输入数据 ``` d ``` (data.frame) 按行（row）或列(col) 遍历，每次取出一行或列，赋予 ``` row ``` 这个变量（可随意取名）；
4. ``` foreach ``` 将 数据 ``` row ``` 分发给 ``` cl ``` （这里没有体现出来），进行计算 ```  row$x * row$y ``` ，并返回结果
5. ``` .combine = 'c' ``` 参数规定将返回结果合并为 ``` vector ``` 。
